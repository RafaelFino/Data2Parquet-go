...
func GenerateLog(pid int, count int, result chan domain.Record, wg *sync.WaitGroup, parallel int) {
	...
	for i := 0; i < count; i++ {
		...
		line := &domain.Log{
			...
			BusinessCapability:          "buscap" + fmt.Sprintf("%02d", i%10),
			BusinessDomain:              "busdom" + fmt.Sprintf("%02d", i%10),
			BusinessService:             "bussvc" + fmt.Sprintf("%02d", i%20),
			ApplicationService:          "appsvc" + fmt.Sprintf("%02d", i%30),
			CorrelationId:               getID(),
			SessionId:                   getID(),
			MessageId:                   getID(),
			PersonId:                    getID(),
